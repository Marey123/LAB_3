...
std::vector<FigureProperties*> Aggregate::getFigures()
{
	std::vector<FigureProperties*> clear_figures;

	for (Figure* figure : figures)
	{
		if (figure->getType() != "Aggregate")
		{
			clear_figures.push_back(dynamic_cast<FigureProperties*>(figure));
		}
		else
		{
			std::vector<FigureProperties*> temp_figures = dynamic_cast<Aggregate*>(figure)->getFigures();
...
int Aggregate::containsFigure(Figure* figure)
{
	for (int index = 0; index < figures.size(); ++index)
	{
...
{
	//AggregateMemento memento(this, filename);
...
void Aggregate::restore(FigureMemento* memento)
{
	...
	figures = *memento.getAggregate()->getFigures();*/
}
